...
def register_callbacks(app):
    # Pong button click event to launch BLE_pong.py
    @app.callback(
    ...
    def launch_pong(n_clicks):
        if n_clicks > 0:
            # Launch pong.py
            subprocess.Popen(['python', 'BLE_pong.py'])  # Replace with actual path to pong.py
        return 0  # Reset the button clicks after launching the game

    # Flappy Bird button click event to launch BLE_flappy.py
    @app.callback(
    ...
    def launch_flappy(n_clicks):
        if n_clicks > 0:
            ...
            flappy_script_path = os.path.join('BLE_FlappyBird', 'BLE_flappy.py')
