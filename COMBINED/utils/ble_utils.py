...
class BLECommunicator:
    def __init__(self):
        ...
        self._is_closing = False  # Initialize before starting the thread
    ...
    def _run_event_loop(self):
        ...
        # Close the loop after it has been stopped
        self.loop.close()
    ...
    def close_connection(self):
        with self.connection_lock:
            if self._is_closing:
                logging.debug("BLE connection is already closing or closed.")
        ...
        try:
            ...
            # Stop the event loop; do not close it here
            if self.loop:
                if self.loop.is_running():
                    self.loop.call_soon_threadsafe(self.loop.stop)
                # Do not attempt to close the loop here
                # It will be closed in _run_event_loop after run_forever exits
